...
import type { IPondBase, IPond, ITreatment } from '@shared/models';
...
export class PondService {
  ...
  @LogFunction()
  public async getPond(entityManager: EntityManager, pondId: Id): Promise<IPond> {
    const repository = entityManager.getRepository(PondEntity);
    const entity = await repository.findOneByOrFail({ id: pondId });
  ...
  @LogFunction()
  public async addPond(entityManager: EntityManager, newPond: IPondBase): Promise<IPond> {
    const repository = entityManager.getRepository(PondEntity);
  ...
  @LogFunction()
  public async updatePond(entityManager: EntityManager, pond: IPond): Promise<IPond> {
    const repository = entityManager.getRepository(PondEntity);
    const existingPond = await repository.findOneByOrFail({ id: pond.id });
  ...
  @LogFunction()
  public async deletePond(entityManager: EntityManager, pond: IPond): Promise<void> {
    const treatments = await this.treatmentService.getTreatments(entityManager, pond.id);
  ...
  private mapEntityToModel(entity: PondEntity, treatments: ITreatment[]): IPond {
    return {
      id: entity.id,
