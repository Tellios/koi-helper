...
export const logger = winston.createLogger({
  level: isProduction ? 'verbose' : 'debug',
  transports,
  format: combine(
    colorize(),
...
export function LogFunction(options?: Partial<ILogFunctionOptions>): MethodDecorator {
  const opts = {
    ...{
      logStart: true,
      logEnd: false,
  ...
  return (
    ...
    descriptor.value = function (...args: any[]) {
      ...
      if (opts.logEnd) {
        logger.log(opts.level, `${targetName}.${propertyKey.toString()} - call ended`);
      }
