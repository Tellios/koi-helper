...
export class TreatmentService {
  ...
  public async add(
    ...
    newTreatment.disease = await diseaseRepository.findOneByOrFail({ id: disease });

    const saved = await repository.save(newTreatment);

    return this.mapTreatmentToModel(saved, await this.getComments(entityManager, saved.id));
  }

  @LogFunction()
  public async update(entityManager: EntityManager, treatment: ITreatment): Promise<ITreatment> {
    const repository = entityManager.getRepository(TreatmentEntity);
    const diseaseRepository = entityManager.getRepository(DiseaseEntity);

    const entity = await repository.findOneByOrFail({ id: treatment.id });
    entity.reference = treatment.reference;
    entity.finished = treatment.finished;
    entity.ended = treatment.ended;
    entity.disease = await diseaseRepository.findOneByOrFail({ id: treatment.diseaseId });
  ...
  @LogFunction()
  public async delete(entityManager: EntityManager, treatmentId: Id): Promise<void> {
    await this.imageService.deleteImagesForReference(entityManager, treatmentId);
  ...
  @LogFunction()
  public async deleteComment(entityManager: EntityManager, commentId: Id): Promise<void> {
    const repository = entityManager.getRepository(TreatmentCommentEntity);
    await repository.delete(commentId);
  }

  private mapTreatmentToModel(entity: TreatmentEntity, comments: ITreatmentComment[]): ITreatment {
    return {
      id: entity.id,
  ...
  private mapCommentEntityToModel(entity: TreatmentCommentEntity): ITreatmentComment {
    return {
      id: entity.id,
      created: entity.created,
      updated: entity.updated,
