...
import type { IFishBase, IFish, IMeasurement, ITreatment } from './models';
...
export class FishService {
  ...
  @LogFunction()
  public async add(entityManager: EntityManager, fish: IFishBase, pondId: Id): Promise<IFish> {
  ...
  @LogFunction()
  public async update(entityManager: EntityManager, fish: IFish): Promise<IFish> {
    const repository = entityManager.getRepository(FishEntity);

    const entity = await repository.findOneByOrFail({ id: fish.id });
    entity.born = fish.born;
    entity.breeder = fish.breeder;
    entity.origin = fish.origin;
    entity.name = fish.name;
  ...
  @LogFunction()
  public async delete(entityManager: EntityManager, fish: IFish): Promise<void> {
